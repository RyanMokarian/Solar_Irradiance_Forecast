...
class GHIs(object):
    ...
    def exists(self, timestamp: datetime):
        if timestamp not in self.df.index:
            raise Exception(f'Timestamp \"{timestamp}\" not found in the metadata dataframe. Every timestamps are supposed to be there.')
...
class Images(object):
    ...
    """
    def __init__(self, images_path: ImagePaths, image_size: int):
    ...
    def yield_time(self):
        for time in images_path.get_timestamps():
            if images_path.exists(time):
                yield time
